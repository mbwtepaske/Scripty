...
namespace Scripty.MsBuild
{
  public class ScriptyTask : Microsoft.Build.Utilities.Task
  {
    ...
    public override bool Execute()
    {
      ...
      var scriptFiles = string.Join(" ", ScriptFiles
      ...
      var process = new Process
      {
        StartInfo =
        {
          FileName = Path.Combine(Path.GetDirectoryName(typeof(ScriptyTask).Assembly.Location), "Scripty.exe"),
          Arguments = arguments,
          CreateNoWindow = true,
      ...
      // Report any errors
      foreach (var error in errorData.Where(x => !string.IsNullOrWhiteSpace(x)))
      {
        Log.LogError(error);
      }

      // Add the compile files
      var outputFiles = outputData
      ...
      _noneFiles.AddRange(outputFiles
          ...
          .Select(x =>
          {
            var taskItem = new TaskItem(x.Item2);
